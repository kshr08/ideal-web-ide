...
// ====================================================================
// File System Access Bridge (runtime)
// ====================================================================
...
window.initFileSystemAccess = async function () {
  ...
  // Store root folder name so React UI can read it
...
window.writeFile = async function (path, content) {
  if (!currentDirHandle) throw new Error("No folder opened");
...
window.movePath = async function (path, destDirPath) {
  ...
  try {
    ...
    console.error("movePath error:", e);
  }
};
...
window.fsUndo = async function () {
  ...
  try {
    if (op.op === "write") {
      await window.writeFile(op.path, op.prev ?? "");
    } else if (op.op === "mkdir") {
      await window.deletePath(op.path);
    } else if (op.op === "delete") {
      if (op.snapshot?.type === "file") {
        await window.writeFile(op.path, op.snapshot.content);
      } else if (op.snapshot?.type === "dir") {
        await window.createDirectory(op.path);
      }
...
window.fsRedo = async function () {
  ...
  try {
    if (op.op === "write") {
      await window.writeFile(op.path, op.next ?? "");
    } else if (op.op === "mkdir") {
      await window.createDirectory(op.path);
    } else if (op.op === "delete") {
      await window.deletePath(op.path);
